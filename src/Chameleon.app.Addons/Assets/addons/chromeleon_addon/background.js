...
async function applyOverrides(tabId) {
  try {
    ...
    await chrome.debugger.sendCommand({tabId: tabId}, 'Emulation.setTimezoneOverride', {
      timezoneId: settings.timezoneId
    });
    await chrome.debugger.sendCommand({tabId: tabId}, 'Emulation.setLocaleOverride', {
      locale: settings.locale
    });
    console.log(`Debugger attached and overrides applied for tab ${tabId}`);
    return true;
  } catch (error) {
    console.error(`Failed to attach debugger or apply overrides for tab ${tabId}:`, error);
    return false;
  }
}
